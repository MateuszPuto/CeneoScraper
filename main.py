...
opinionList = []
respons = requests.get("https://www.ceneo.pl/22496575")
page = 0

while respons.status_code == 200:
    page += 1
    respons = requests.get("https://www.ceneo.pl/22496575/opinie-"+str(page), allow_redirects=False)
    ...
    for opinion in opinions:
        ...
        try:
            ...
            purchased = False
        publishDate = opinion.select("span.user-post__published > time:first-child").pop(0)["datetime"].strip()
        try:
...
with open("./opinions/22496575.json", "w", encoding="UTF-8") as f:
    json.dump(opinionList[:-1], f, indent=4, ensure_ascii=False)